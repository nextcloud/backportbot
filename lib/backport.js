...
module.exports = async function (context, targets, logger) {
  ...
  for (const target of targets) {
    ...
    try {
      ...
      // Check conflicts
      const oldFiles = await pullRequest.getFiles(context)
      const newFiles = await pullRequest.getFiles(context, newPrId)
      const conflicts = oldFiles.length !== newFiles.length
      logger.debug('Checking conflicts', oldFiles.length, newFiles.length)
      if (conflicts) {
