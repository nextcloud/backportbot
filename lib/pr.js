module.exports = {
  ...
  getChanges: async function (context, issueId) {
    const number = this.getNumber(context)
    const params = !issueId
      ? context.repo({ pull_number: number })
      : context.repo({
  ...
  updatePRBody: async function (context, issueId, body) {
    const params = context.repo({
      pull_number: issueId,
      body: body
    })
    return context.github.pulls.update(params)
  },
  ...
  backportFailed: async function (context, target, error) {
    const errorName = error && error.prototype && error.prototype.name
...
git checkout -b fix/foo-${target.branch}
...
git push origin fix/foo-${target.branch}
\`\`\`
...
More info at https://docs.nextcloud.com/server/latest/developer_manual/getting_started/development_process.html#manual-backport`
  ...
  getReviewers: async function (context) {
    ...
    const reviewers = reviews.data.filter(review => review.state === 'APPROVED')
    return reviewers.map(reviewer => reviewer.user.login)
  },
